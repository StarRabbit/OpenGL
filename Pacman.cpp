...
int main(int argc, char** argv){
    ...
    glutInitWindowSize(2560, 1600);                    //窗口大小
    glutInitWindowPosition(50, 50);                //窗口起始位置
    glutCreateWindow("试一试 - by Van");

    //定义所有控制功能，这些注册的函数指针
    //这个画图函数只在显示窗口时被调一次
    // 如果想多次调可以注册一个glutPostRedisplay
    glutDisplayFunc(display);            //显示窗口
    glutReshapeFunc(reshape);            //重置窗口
    //这个函数是动画控制的重点
    glutIdleFunc(idledisplay);                //循环
